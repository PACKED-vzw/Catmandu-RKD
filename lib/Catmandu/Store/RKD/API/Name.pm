...
sub __search {
    ...
    if (defined($startIndex)) {
        my $template = 'http://opendata.rkd.nl/opensearch/artists/eac-cpf?q=naamdeel:(%s)&startIndex=%s';
        $url = sprintf($template, $self->name_to_search, $startIndex);
    } else {
        my $template = 'http://opendata.rkd.nl/opensearch/artists/eac-cpf?q=naamdeel:(%s)';
        $url = sprintf($template, $self->name_to_search);
    }
